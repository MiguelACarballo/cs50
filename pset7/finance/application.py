...
def history():
    ...
    curr_username = db.execute("SELECT username FROM users WHERE id = :session_id", session_id=session["user_id"])
    result = db.execute("SELECT * FROM transactions WHERE username = :username", username=curr_username[0]["username"])
    if not result:
        return apology("there are no transactions yet")
    for price in result:
        price["price"] = usd(float(price["price"]))
    return render_template("history.html", stocks=result)
...
def sell():
    ...
    if request.method == "POST":
        ...
        if not user_shares:
            ...
            # insert transaction into transactions table
            db.execute("INSERT INTO transactions (username, price, symbol, shares, kind) VALUES (:username, :price, :symbol, :shares, 'Sold')", \
            username=curr_username[0]["username"], price=curr_stock["price"] * curr_shares, \
            symbol=curr_stock["symbol"], shares=curr_shares)
        else:
            curr_stock = lookup(request.form.get("symbol"))
